...
const BookReader = ({ book, savedWords }) => {
  ...
  return (
    ...
    >
      <article className="w-9/12 mx-auto">
        <h1 className={`text-4xl text-center my-8 font-semibold ${oldFont ? 'old-font' : 'modern-font'}`}>
          {book.title}
        </h1>
        <h2 className={`text-3xl text-center my-6 font-medium ${oldFont ? 'old-font' : 'modern-font'}`}>{book.author}</h2>
        <div className={`${selectTextSize()} leading-10 ${oldFont ? 'old-font' : 'modern-font'}`}>{hiragana ? book.text : book.textNoHiragana}</div>
      </article>

      <Draggable>
        <div className="fixed top-1/2 left-2 bg-black opacity-90 p-5 flex flex-col gap-3 rounded-lg border-2 border-white">
      ...
      <Draggable>
        <div className="flex flex-col items-center gap-2 bg-black opacity-90 rounded-lg border-white border-2 max-h-[70vh] overflow-auto fixed right-2 top-1/2 p-3 min-w-40">
          {savedWords.map(word => (
            <div key={word} className="text-white">{word}</div>
          ))}
        </div>
      </Draggable>
    </div>
