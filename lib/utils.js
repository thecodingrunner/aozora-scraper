...
export async function checkFrequency(text) {
    ...
    const filePath = path.join(process.cwd(), 'public', 'kanjiFreqList.json');
    const jsonData = fs.readFileSync(filePath, 'utf-8');
    const data = JSON.parse(jsonData);
    ...
    function findIncludedWords(paragraph, frequencyList) {
        let remainingKanji;
        numberOfWords = Math.round(paragraph.length)
        ...
        frequencyList.forEach(word => {
          ...
          includedItems[word[0]] = [count, word[1]];
        ...
        frequencyScore = frequencyScore + (remainingKanji.length * 2300)
        ...
        frequencyList.forEach(word => {
            comparisonScore += numberOfWords * (1/(word[1])/calcVal) * (word[1])
        })
        // console.log(frequencyList.length)
        // comparisonScore = frequencyList.length * numberOfWords * (1/calcVal)

        freqScore = Math.round((frequencyScore / (comparisonScore*2))*100)
        console.log(comparisonScore, freqScore, frequencyScore)
    }

    findIncludedWords(text, data)

    let numberOfKanji = numberOfWords

    return {includedItems, freqScore, numberOfKanji}
}
